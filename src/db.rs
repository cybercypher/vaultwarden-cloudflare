...
pub async fn query_one<T: DeserializeOwned>(db: &D1Database, query: &str, params: &[JsValue]) -> Result<Option<T>> {
    let stmt = db.prepare(query);
    let stmt = stmt.bind(params).map_err(|e| Error::internal(format!("D1 bind error: {e}")))?;
    stmt.first::<T>(None).await.map_err(|e| Error::internal(format!("D1 query error: {e}")))
}

/// Execute a D1 query that returns multiple rows.
pub async fn query_all<T: DeserializeOwned>(db: &D1Database, query: &str, params: &[JsValue]) -> Result<Vec<T>> {
    let stmt = db.prepare(query);
    let stmt = stmt.bind(params).map_err(|e| Error::internal(format!("D1 bind error: {e}")))?;
    let result = stmt.all().await.map_err(|e| Error::internal(format!("D1 query error: {e}")))?;
    result.results::<T>().map_err(|e| Error::internal(format!("D1 deserialize error: {e}")))
}

/// Execute a D1 statement that doesn't return rows (INSERT, UPDATE, DELETE).
pub async fn execute(db: &D1Database, query: &str, params: &[JsValue]) -> Result<()> {
    let stmt = db.prepare(query);
    let stmt = stmt.bind(params).map_err(|e| Error::internal(format!("D1 bind error: {e}")))?;
    stmt.run().await.map_err(|e| Error::internal(format!("D1 execute error: {e}")))?;
    Ok(())
}
