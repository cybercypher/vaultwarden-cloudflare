...
fn get_authority(env: &Env) -> Result<String> {
    env.var("SSO_AUTHORITY")
        .map(|v| v.to_string())
        .map_err(|_| Error::internal("SSO_AUTHORITY not configured"))
}

fn get_client_id(env: &Env) -> Result<String> {
    env.var("SSO_CLIENT_ID")
        .map(|v| v.to_string())
        .map_err(|_| Error::internal("SSO_CLIENT_ID not configured"))
}
...
async fn discover_oidc(authority: &str) -> Result<Value> {
    ...
    let mut resp = worker::Fetch::Request(req)
        .send()
        .await
        .map_err(|e| Error::internal(format!("Discovery fetch error: {e}")))?;
    ...
    resp.json::<Value>()
        .await
        .map_err(|e| Error::internal(format!("Discovery parse error: {e}")))
}
...
pub async fn authorize(req: Request, env: &Env) -> Result<Response> {
    ...
    let state = url.query_pairs()
        .find(|(k, _)| k == "state")
        .map(|(_, v)| v.to_string())
        .unwrap_or_else(|| util::get_uuid());
    let code_challenge = url.query_pairs()
        .find(|(k, _)| k == "code_challenge")
        .map(|(_, v)| v.to_string());
    let code_challenge_method = url.query_pairs()
    ...
    if let Ok(kv) = env.kv("KV") {
        ...
        let _ = kv.put(&format!("sso_state_{}", state), &state_data.to_string())
...
pub async fn oidc_signin(req: Request, env: &Env) -> Result<Response> {
    ...
    if let Ok(kv) = env.kv("KV") {
        let stored = kv.get(&format!("sso_state_{}", state)).text().await
            .map_err(|e| Error::internal(e.to_string()))?;
        if stored.is_none() {
    ...
    let token_endpoint = discovery["token_endpoint"]
        .as_str()
        .ok_or_else(|| Error::internal("Missing token_endpoint"))?;
    ...
    let token_data: Value = token_resp.json().await
        .map_err(|e| Error::internal(format!("Token parse error: {e}")))?;

    let id_token = token_data["id_token"]
        .as_str()
        .ok_or_else(|| Error::internal("Missing id_token"))?;
    ...
    let id_claims: Value = serde_json::from_slice(&payload_bytes)
        .map_err(|_| Error::internal("Invalid id_token payload"))?;

    let email = id_claims["email"]
        .as_str()
        .ok_or_else(|| Error::internal("id_token missing email claim"))?
        .to_lowercase();
    let sso_sub = id_claims["sub"]
        .as_str()
        .ok_or_else(|| Error::internal("id_token missing sub claim"))?;
    ...
    let mut user = if let Some(existing) = User::find_by_email(&email, &d1).await? {
        ...
        new_user.name = id_claims["name"]
            .as_str()
            .or(id_claims["preferred_username"].as_str())
            .unwrap_or(&email)
            .to_string();
    ...
    let access_validity: i64 = env.var("ACCESS_TOKEN_VALIDITY")
        .map(|v| v.to_string().parse().unwrap_or(7200)).unwrap_or(7200);
    let refresh_validity_days: i64 = env.var("REFRESH_TOKEN_VALIDITY_DAYS")
        .map(|v| v.to_string().parse().unwrap_or(30)).unwrap_or(30);

    let access_claims = auth::make_login_claims(
        &user.uuid, &user.email, &user.name, &device.uuid,
        &Device::type_to_string(device.atype), &user.security_stamp,
        "web", scope.clone(), access_validity, &domain,
    );
    let refresh_claims = auth::make_refresh_claims(
        &user.uuid, &device.uuid, &device.refresh_token,
        scope, refresh_validity_days, &domain,
    );
...
pub async fn sso_login(
    ...
    let token_body = serde_urlencoded::to_string(&params)
        .map_err(|e| Error::internal(format!("URL encode error: {e}")))?;
    ...
    let email = id_claims["email"].as_str()
        .ok_or_else(|| Error::internal("id_token missing email"))?
        .to_lowercase();
